...
일반적인 JWT 방식은 Refresh 토큰을 도입하더라도, 이미 발행된 Access 토큰에 대한 즉각적인 무효화가 불가능합니다. 서버는 토큰 자체의 유효성만 검증할 뿐 매 요청마다 상태를 확인하지 않기 때문에, 토큰이 탈취되거나 보안 정책 위반이 감지되어도 해당 토큰이 만료될 때까지 서버는 제어권을 상실하는 사각지대가 발생합니다. CMS 관리자에게 필요한 것은 "5분 뒤에 만료되는 토큰"이 아니라, "지금 당장 차단되는 세션" 입니다. 물론 JWT도 블랙리스트를 추가 구현하여 제어할 순 있으나, 이는 무상태(Stateless)라는 JWT의 이점을 희생시키고 시스템 복잡도를 높이는 결과라고 생각했습니다. 즉, 서버가 매 요청마다 "이 토큰이 차단되었나?"를 확인하기 위해 Redis나 DB를 조회하게 된다면, 그것은 이미 무상태가 아닌 상태 유지 시스템이 된 것이라고 생각합니다. 이런 이유들로 본 프로젝트는 JWT를 쓰고 억지로 상태를 관리하는 방식 대신, 처음부터 세션을 외부 DB에 저장하는 직관적인 설계를 선택했습니다.

**3. 향후 기능 계획**

마지막으로 향후 기능을 더 만들고 싶다면 사용자가 현재 로그인된 모든 기기(PC, 모바일 등)의 리스트를 확인하고, 보안이 우려되는 특정 기기나 공공 장소의 세션을 원격으로 즉시 종료할 수 있는 기능, 사용자의 접속 환경(IP 주소, 브라우저 정보 등)이 세션 생성 시점과 급격하게 달라질 경우, 이를 실시간으로 감지하여 보안을 강화하는 기능을 구현해보고 싶습니다.
